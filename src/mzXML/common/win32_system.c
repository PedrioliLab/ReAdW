...
static char *process_cmd_for_cygwin(const char *in_cmd) {
   ...
   while (next && *next) {
      ...
      struct stat info;
      char *space, *binpath, *full, *newcmd; 
...
static char *setComSpec() {
	...
	char *cp;
	struct stat info;
	char *newComSpec = (char *)malloc(strlen("COMSPEC=")+strlen(CGI_FULL_BIN)+strlen("cmd.exe")+3);
	if (cp=getenv("COMSPEC")) {
		oldComSpec=strdup(cp);
	} else if (cp=getenv("ComSpec")) {
		oldComSpec=strdup(cp);
	} 
	sprintf(newComSpec,"%scmd.exe",CGI_FULL_BIN);
	if (oldComSpec && (-1==stat(newComSpec,&info))) {
		copy_file(oldComSpec,newComSpec);
	}
	sprintf(newComSpec,"COMSPEC=%scmd.exe",CGI_FULL_BIN);
...
static char *process_command_for_windows(const char *cmd) {
   ...
   buf = (char *)malloc(strlen(result)+strlen(CGI_FULL_BIN)+8);
   // do we need to add the cgi path? or is this a shell cmd like "dir"?
   strcpy(buf,CGI_FULL_BIN);
   fixpath(buf); // get those slashes pointing left
   strcat(buf,result);
   space = strchr(result,' ');
   if (space) { // cut off the arglist
      *(buf+strlen(CGI_FULL_BIN)+(space-result))=0;
   }
   if (!stat(buf,&statbuf)) {
