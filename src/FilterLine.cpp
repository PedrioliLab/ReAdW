...
supplementalActivation_(BOOL_UNDEF),
...
FilterLine::print() {
	...
	if (supplementalActivation_ != BOOL_UNDEF) {
		cout << "supplemental activation: " << supplementalActivation_ << endl;
	}
...
FilterLine::parse(string filterLine) {
	...
	*/
	...
	// synchronus precursor selection
	if ((w == "!SPS") || (w == "SPS")) {
		s >> w;
	}
	...
	if (w == "!SA") {
		supplementalActivation_ = BOOL_FALSE;
		advance = true;
	}
	else if (w == "SA") {
		supplementalActivation_ = BOOL_TRUE;
		advance = true;
	}
	if (advance) {
		if (s.eof()) {
